...
class mytsa(object):
    ...
    def register_client(self, clientId, size):
        if clientId not in self.client_info:
            ...
            self.client_info[clientId]['size'] = size
    ...
    def update_client(self, clientId, duration, update):
        ...
        if self.client_info[clientId]['beta'] + self.client_info[clientId]['alpha'] > 10:
            self.client_info[clientId]['alpha'] = 1
            self.client_info[clientId]['beta'] = 1

    def calculate_similarity(self, global_model, local_models_param):
        ...
        dataset = NoiseDataset([self.args.num_channels, self.args.dim, self.args.dim], self.args.num_samples)
    ...
    def select_participant(self, select_num, durations):
        ...
        for clients in client_list:
            ...
            score *= math.sqrt(self.client_info[clients]['size'])
            scores.append(score)
